...
namespace ShareMarket.BusinessLogic.Message
{
    ...
    public partial class Task
    {
        ...
        private ITask CreateTask()
        {
            ...
            if (this.Enabled)
            {
                ...
                if (type2 != null)
                {
                    ...
                    object instance = Activator.CreateInstance(type2) as ITask;
                    task = instance as ITask;
                }
            }
            return task;
        ...
        public void Execute()
        {
            ...
            var scheduleTask = GlobalLib.GetTaskByType(this.Type);
            ...
            {
                using (
                    IDataRepository<ScheduleTask> ScheduleTaskContext =
                        GlobalUtil.Container.Resolve<IDataRepository<ScheduleTask>>())
                {
                    //update appropriate datetime properties
                    scheduleTask.LastEndUtc = this.LastEndUtc;
                    scheduleTask.LastSuccessUtc = this.LastSuccessUtc;
                    ScheduleTaskContext.Update(scheduleTask);
                    ScheduleTaskContext.SaveChanges();
                   // ScheduleTaskManager.Instance.Save(scheduleTask);
                    //scheduleTaskService.UpdateTask(scheduleTask);
                }
            }
